...
| 项 | 内容 | severity | 回退 |
|---|---|---|---|
| PPT-01 | 页数在档位区间 | 高 | S2（预算失衡）或 S1（档位选错） |
| PPT-02 | 每页要点数 ≤ 上限 | 高 | S3 |
| PPT-03 | 每页可见字数 ≤ 上限 | 高 | S3 |
| PPT-04 | 无估算文本溢出 | 严重 | S3（删字/拆页）或 S4（调字号） |
| PPT-05 | 实测颜色 ∈ 主题调色板 | 高 | S4 |
| PPT-06 | 同版式标题位置一致 | 一般 | S5（引擎问题，报 bug） |
...
- 每个 FAIL 挂 open_issue（category/target_stage 按上表），修复后重跑 QA，PASS 才关闭。
- PPT-04 是引擎估算（CJK≈1em、西文≈0.52em 折行模型），存在误差；用户肉眼复核发现误报/漏报时先核字符宽度系数，不得直接放宽阈值。
