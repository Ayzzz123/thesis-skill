...
原则：文档写 A、代码实现 B → FAIL。检查对象：
  DOC-01 SKILL/README 版本声明（v1.5.0 为正式版；v1.6.0 仅以开发态出现，且 SKILL 明示未发布）
  DOC-02 CHANGELOG [Unreleased] 顶部（v1.6.0 不得写成已发布条目）
...
def main():
    ...
    check("DOC-01 SKILL 正式版本=1.5.0（v1.6 未提前转正）",
          v_skill and v_skill.group(1) == "1.5.0", str(v_skill and v_skill.group(1)))
    skill_f = flat(skill)
    check("DOC-01 SKILL 明示 v1.6.0 开发中未发布",
          "v1.6.0 开发中" in skill_f and "feature-1.6.0-full-stack-orchestration" in skill)
    check("DOC-01 README 当前版本声明仍为 aeromech-thesis v1.5.0",
          re.search(r"aeromech-thesis v1\.5\.0", readme) is not None)
    check("DOC-01 README 含 v1.6.0 development/roadmap 且声明 Phase6/7 未完成",
          "v1.6.0 development" in readme and "Phase 6" in readme
          and "未发布" in readme or "roadmap" in readme)
    check("hard README/SKILL 不得把 v1.6.0 写成已实现发布能力（§十五 原则）",
          "aeromech-thesis v1.6.0" not in readme)   # 正式版本标题行禁止出现 v1.6.0

    # ---------- DOC-02 CHANGELOG ----------
    head = changelog.split("\n## ", 2)
    check("DOC-02 CHANGELOG 顶部条目=[Unreleased]（v1.6.0 不写成正式 release）",
          head[1].startswith("[Unreleased]"), head[1][:30])
    check("DOC-02 v1.5.0 仍为最新正式发布条目",
          re.search(r"^## v1\.5\.0", changelog, re.M) is not None)
    clg_f = flat(changelog)
    check("DOC-02 [Unreleased] 内声明 Phase6/7 未执行且版本号未升级",
          "Phase 6" in clg_f and "Phase 7" in clg_f
          and "正式版本号仍为 v1.5.0" in clg_f)
    ...
    check("DOC-02 CHANGELOG 最新 release 与 README/SKILL 版本联动一致",
          cm and rm and v_skill and cm.group(1) == rm.group(1) == v_skill.group(1) == "1.5.0")
