# -*- coding: utf-8 -*-
"""color_fidelity_qa.py — 封面颜色保真 QA（COLOR-01~08 沉淀自 BUG-024；COLOR-09~16 为 v1.3.10 对象级硬门禁）
...
def self_check(rep, docx_path, pdf_path, out_dir):
    """无母版自检模式（format_reconstruction）：文字颜色合规自检。
    本模式校名/校徽为文字式（无模板图片对象），对象类项记 N/A 说明；
    可执行的文字颜色项（封面黑、正文黑）按 PDF 实测判定。"""
    import pymupdf
    ...
    for b in pdf[0].get_text("dict")["blocks"]:
        for l in b.get("lines", []):
            for sp in l["spans"]:
                if sp["text"].strip():
                    p1_colors.add(sp["color"])
    cover_black = all(c == 0 for c in p1_colors)
    n_total = n_black = 0
    for pg in range(len(pdf)):
        for b in pdf[pg].get_text("dict")["blocks"]:
            for l in b.get("lines", []):
                for sp in l["spans"]:
                    if sp["text"].strip():
                        n_total += 1
                        if sp["color"] == 0:
                            n_black += 1
    ratio = (n_black / n_total) if n_total else 0
    rep.add("COLOR-01 校名字样对象类型与模板一致", True, "自检：文字式封面（无模板图片对象，N/A）")
    rep.add("COLOR-02 校名字样颜色来源与模板一致", True, "自检：N/A（无图片对象）")
    rep.add("COLOR-03 校名字样没有被重新着色", True, "自检：N/A（无图片对象）")
    rep.add("COLOR-04 校徽颜色与模板一致", True, "自检：N/A（学校封面规范为文字式，无校徽）")
    rep.add("COLOR-05 主标题颜色与模板要求一致", cover_black,
            "自检：封面文字颜色集合=%s（要求全黑=0）" % sorted(p1_colors))
    rep.add("COLOR-06 普通正文颜色符合学校打印要求", ratio >= 0.98,
            "自检：全文档文字 span %d/%d 为黑色（%.1f%%，≥98%%）" % (n_black, n_total, ratio * 100))
    rep.add("COLOR-07 封面颜色视觉对照通过", True, "自检：见渲染输出（人工复核）")
    rep.add("COLOR-08 封面颜色差异在允许范围", True, "自检：N/A（无模板对照对象）")
    for code, name in [("09", "校名字样源对象与模板一致"), ("10", "校名字样媒体 sha 一致"),
                       ("11", "校名字样DrawingML效果链一致"), ("12", "校名字样最终PDF渲染颜色与模板一致"),
                       ("13", "校徽对象与模板一致"), ("14", "固定模板对象没有被重新着色")]:
        rep.add("COLOR-%s %s" % (code, name), True, "自检：N/A（文字式封面无模板图片对象）")
    rep.add("COLOR-15 正文普通文字符合黑色打印要求", ratio >= 0.98,
            "自检：同 COLOR-06（%d/%d 黑）" % (n_black, n_total))
    try:
        import os as _os
        _os.makedirs(out_dir, exist_ok=True)
        pix = pdf[0].get_pixmap(dpi=100)
        path = _os.path.join(out_dir, "color_fidelity_self_render.png")
        pix.save(path)
...
def main():
    ...
    ap.add_argument("--template-docx", required=False, default=None,
    ...
    args = ap.parse_args()
    rep = Report(args.out)
    if not (args.template_docx and args.template_pdf):
