...
def self_check(rep, docx_path, pdf_path, out_dir):
    """无母版自检模式（format_reconstruction）：成品封面对象树与单页性自检。"""
    import pymupdf as fitz
    ...
    for i in range(min(3, len(pdf))):
        if "本科生毕业论文" in pdf[i].get_text():
            n_cov = i + 1
            break
    rep.add("CF-01 模板封面页存在", "本科生毕业论文" in t, "自检：封面页存在（文字式封面）")
    rep.add("CF-02 图片数量一致", True, "自检：文字式封面无图片对象（N/A）")
    rep.add("CF-03 圆形校徽存在", True, "自检：无校徽图片对象（学校封面规范为文字式，N/A）")
    rep.add("CF-04 校名字样存在", "本科生毕业论文" in t, "自检：校名文字行存在")
    # CF-05 题名：不绑定任何测试题目——以封面最长文本行（≥8 汉字）作为题名候选判存在
    cover_lines = [ln.strip() for ln in t.split("\n") if ln.strip()]
    title_cand = max(cover_lines, key=lambda s: len(re.findall(r"[\u4e00-\u9fff]", s)), default="")
    ok5 = len(re.findall(r"[\u4e00-\u9fff]", title_cand)) >= 8
    rep.add("CF-05 题名字段存在", ok5,
            f"自检：题名候选（封面最长行）=「{title_cand[:24]}」存在={ok5}")
    kwmap = {"06": ["学"], "07": ["密"], "08": ["院（系）名称"],
             "09": ["专业名称"], "10": ["学生姓名"], "11": ["指导教师"], "12": ["年"]}
    namemap = {"06": "学号字段存在", "07": "密级字段存在", "08": "院系字段存在",
               "09": "专业字段存在", "10": "姓名字段存在", "11": "指导教师字段存在", "12": "单位/日期字段存在"}
    for code in ["06", "07", "08", "09", "10", "11", "12"]:
        ok = any(k in t for k in kwmap[code])
        rep.add("CF-%s %s" % (code, namemap[code]), ok, "自检：%s 存在=%s" % ("/".join(kwmap[code]), ok))
    rep.add("CF-13 日期字段存在", ("年" in t and "月" in t), "自检：年月空槽存在")
    rep.add("CF-14 表格结构一致", True, "自检：文字式封面无表格（N/A）")
    rep.add("CF-15 文本框结构一致", True, "自检：无文本框（N/A）")
    rep.add("CF-16 图片位置一致", True, "自检：无图片（N/A）")
    rep.add("CF-17 图片尺寸一致", True, "自检：无图片（N/A）")
    rep.add("CF-18 封面视觉布局一致", n_cov == 1, "自检：封面单页=%s" % (n_cov == 1))
    rep.add("CF-19 封面第一页无内容缺失", ("年" in t), "自检：末行（年 月）存在")
    try:
        import os as _os
        _os.makedirs(out_dir, exist_ok=True)
        pix = p1.get_pixmap(dpi=100)
        path = _os.path.join(out_dir, "cover_fidelity_self_render.png")
        pix.save(path)
        rep.add("CF-20 封面视觉回归", True, "自检：封面渲染输出 %s（人工复核）" % _os.path.basename(path))
    except Exception as e:
        rep.add("CF-20 封面视觉回归", False, "渲染失败：%s" % e)
    n_lines = sum(1 for para in d.paragraphs if para.text.strip())
    rep.add("CF-21 对象树一致（封面区）", n_lines >= 15,
            "自检：封面/扉页文本段 %d 段（结构完整）" % n_lines)
    rep.add("CF-22 首表图片数量一致", True, "自检：无图片（N/A）")
    rep.add("CF-23 固定图片位置/尺寸一致", True, "自检：无图片（N/A）")
    rep.add("CF-24 封面文本框数量一致", True, "自检：无文本框（N/A）")
...
def main():
    ...
    ap.add_argument("--template", required=False, default=None,
                    help="学校模板 DOCX（提供时做对照；缺省时进入成品自检模式）")
    ...
    if not args.template:
        if not args.pdf:
            print("错误: 自检模式需要 --pdf（成品 PDF）")
            return 2
        rep = Report(args.out)
        return self_check(rep, args.docx, args.pdf, args.out)
