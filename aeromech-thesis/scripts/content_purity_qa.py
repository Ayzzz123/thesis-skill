# -*- coding: utf-8 -*-
"""content_purity_qa.py — 内容净化与附录分页 QA（MD/INT/PRM/APX/TABCAP）。通用版：
MD-01 无 Markdown 残留；INT-01 无内部路径泄漏；PRM-01 无提示语/角色词泄漏；
APX-01 附录标题独立起页（按实际存在的附录动态核验）；APX-02 附录表完整性；
TABCAP-01~08 表启动块（TABLE_START_BLOCK）：中英题同页/英题与表头同页/表头与首行同页/
表题不单独留上一页/启动页无大片空白/长表跨页重复表头/图表题注顺序正确/独立图页净化。
题注编号支持"表1-1/表1.1/表A1"（短横/点号/附录序码）。
...
CAP_TAB_RE = re.compile(r"^表(\d+)[.\-](\d+)|^表([A-Z])[.\-]?(\d+)")
...
def main():
    ...
    for kw in ["artifacts/", ".aeromech", "transcripts/", "state.yaml", "materials.yaml",
               "qa-report", "build_docx", "chapters/", "figures/", "computation/",
    ...
    # APX-01 附录标题独立起页（按论文实际存在的附录动态核验）
    app_titles = []
    for p in d.paragraphs:
        if p.style.name == "Heading 1" and re.match(r"^附录[A-Z]", p.text.strip()):
            app_titles.append(p.text.strip()[:3])
    apx1_ok, apx_detail = True, []
    for marker in app_titles:
        found = False
        for i, t in enumerate(pages):
    ...
    if not app_titles:
        rep.add("APX-01 附录标题独立起页", True, "本论文无附录", skip=True)
    else:
        rep.add("APX-01 附录标题独立起页", apx1_ok, "；".join(apx_detail))

    # APX-02 附录表完整性（单页或跨页重复表头；表头签名按实际表）
    body = d.element.body
    ...
    for el in els:
        if el is app_el:
            seen = True
            continue
        if seen and el.tag == qn("w:tbl") and "w:drawing" not in el.xml:
            app_tbls.append(el)
    ...
    for tel in app_tbls:
        ...
        if len(rows) < 2:
            continue
        hdr0 = "".join(x.text or "" for x in rows[0].iter(qn("w:t")))
        hdr_sig = nx(hdr0)[:8]
        first = "".join(x.text or "" for x in rows[1].iter(qn("w:t")))[:12]
        last_row_txt = "".join(x.text or "" for x in rows[-1].iter(qn("w:t")))[:10]
        pg_first = pg_last = None
        for i, t in enumerate(pages):
            nt = nx(t)
            if nx(first)[:6] and nx(first)[:6] in nt and pg_first is None:
                pg_first = i
    ...
    if not app_tbls:
        rep.add("APX-02 附录表完整性", True, "附录无数据表", skip=True)
    else:
        rep.add("APX-02 附录表完整性", apx2_ok,
    ...
    for i, el in enumerate(els):
        ...
        if "w:drawing" in el.xml:
            ...
            for pi, p_el in enumerate(ps):
                ...
                if t and CAP_FIG_RE.match(t) and cap_idx is None:
                    cap_idx = pi
                    cap_txt = t
            if cap_txt:
        ...
        while j >= 0 and steps < 4:
            if els[j].tag == qn("w:p"):
                ...
                if t.startswith("Tab.") and not cap_en:
                    cap_en = t
                elif CAP_TAB_RE.match(t) and not cap_cn:
                    cap_cn = t
                    break
    ...
    for i, el in enumerate(els):
        ...
        if not (CAP_TAB_RE.search(joined) or "Tab." in joined):
            order_ok = False
    ...
    def cap_key(s):
        m = re.match(r"^([表图])\s*(\d+|[A-Z])\s*[.\-]?\s*(\d+)", s)
        if not m:
            return None
        ch = m.group(2)
    ...
    geo = []
    for t in tbls:
        m = re.match(r"^表(\d+[.\-]\d+)", t["cap_cn"]) or re.match(r"^表([A-Z][.\-]?\d+)", t["cap_cn"])
    ...
    # TABCAP-08 独立图页净化（通用）：图占页高≥60% 的图页，除图与题注外无长文本块
    t8_bad = []
    for pat, tag in [(r"（图\d+[.\-]\d+[^）]{60,}", "长图注行残留")]:
    ...
    for p in range(len(doc)):
        for im in doc[p].get_images(full=True):
            for r in doc[p].get_image_rects(im[0]):
                if r.width / 72 * 2.54 > 5 and p > 0:
                    fig_pages.setdefault(p, []).append(r)
    for pg, rects in fig_pages.items():
        H = doc[pg].rect.height
        img_h = sum(r.height for r in rects)
        if img_h / H < 0.60:
            continue  # 图文混排页，不做独立图页净化检查
        others = []
        ...
        if [o for o in others if len(o) > 40] or len(others) > 2:
            t8_bad.append(("独立图页存在多余文本", str((pg + 1, others[:3]))))
    rep.add("TABCAP-08 独立图页净化", not t8_bad,
            f"共 {len(fig_pages)} 个含图页完成净化核验（长文本块=0）"
