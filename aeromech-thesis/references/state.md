...
│   ├── research/                  # v1.4 Research Integrity 注册表（RQ/M/E/DS/AN/CALC/CL/CON/FIG/CONFLICT + traceability.json）
│   ├── artifacts/                 # Skill 产出物
│   │   ├── analysis/              # 工程分析产物
│   │   ├── chapters/              # 论文章节
│   │   ├── figures/               # 图表
...
## 17. Research Integrity 阶段职责（v1.4.0）

注册表位于 `.aeromech/research/`（10 个 YAML + `traceability.json`），规则见 `references/research-integrity.md`，SKILL.md §19。**从 S3 起随阶段写入，不得事后补造**：

| 阶段 | 注册动作 | 引擎命令 |
|---|---|---|
| S3（研究方案） | 建 `rq.yaml`（RQ-XX：question/objective/related_methods/chapters/analyses/conclusions）+ `methods.yaml`（M-XXX：name/basis） | `research_integrity.py <root> init` |
| S4（文献登记） | 文献→`evidence.yaml`（E-XXX，source_type=literature，verification_status 按核实程度） | `add`（或直接编辑 YAML 后 `validate`） |
| S5/S6（分析与数据） | 数据→`datasets.yaml`（DS-XXX；模拟数据必填 reason/assumptions/generation_method/limitations/label）；分析→`analyses.yaml`（AN-XXX，inputs 指向 DS/E/CALC）；关键计算→`computations.yaml`（CALC-XXX：inputs/formula/verification） | `validate` |
| S7（章节写作） | 核心论断→`claims.yaml`（CL-XXX，claim_type + evidence_ids/analysis_ids）；图表→`figures.yaml`（FIG/TABLE-XXX，related_rqs/analyses/claims） | `trace` |
| 结论/摘要完成 | 结论→`conclusions.yaml`（CON-XXX：claims/analyses/evidence） | `trace` + `coverage` |
| S9 前 / Delivery Gate 前 | 跑 RQG 全量；Critical/High 必须清零或挂 open_issue 回退 | `research_quality_qa.py --project <root> [--pdf …]` |

**门禁联动**：
1. `writing.gate_evidence.gate_passed` 从 `conditional` 升 `passed` 前，RQG 不得存在 Critical/High（或存在但已挂 open_issue 且用户知情）。
2. 证据越界（模拟数据支撑强断言结论）、模拟身份丢失、结论断链 → 按 qa 流程挂 open_issue 回退 S6/S7。
3. 无 `.aeromech/research/` 的旧项目：`research_quality_qa.py` 退出码 2 且记 `not_initialized`，不阻塞交付（兼容规则）。
4. **NEEDS_HUMAN_REVIEW（v1.4.1）**：弱证据语境下 RQG-09/RQG-10 的语义项不得自动 PASS——生成
   `artifacts/qa/human-review-checklist.md`，Gate 记 `PASS_WITH_HUMAN_REVIEW`；**交付前**必须按
