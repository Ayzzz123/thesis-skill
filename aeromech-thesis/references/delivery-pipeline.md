...
所有 QA / Registry / Gate 输出使用稳定状态词表（细则见 `research-integrity.md` §19）：

- `PASS` / `WARN` / `FAIL` / `NEEDS_HUMAN_REVIEW`（NHR）/ `NOT_APPLICABLE` / `SKIPPED_WITH_REASON` / `ERROR`；
- **FAIL** 项必须携带：稳定状态码 + severity + reason + remediation；
- **NHR**（启发式无法可靠判定的 Critical 级语义问题，如弱证据语境下的 RQG-09/RQG-10）：
  Gate 记 `PASS_WITH_HUMAN_REVIEW`——允许流程继续，但**交付前必须完成人工复核**
  （`references/research-human-review.md`；裁决回填 `.aeromech/research/human-review.yaml`；violation → FAIL）；
- **NOT_APPLICABLE**：当前项目条件下无法执行的检查（典型：无模板项目的 tf_qa 模板对照项）
  记 N/A，**不计 FAIL、不伪造 PASS**；Delivery Gate 不因"本项目本来就没有模板"而失败；
- **ERROR**（内部异常/注册表损坏）：退出码 3，绝不产生 PASS 结论。

### 8.2 无模板项目的格式 QA（tf_qa）

- 无学校模板/规范文档的项目：`tf_qa.py` 可省略 `--template`（或传 `none`）运行；
  TF-01~04、TF-19 等模板对照项输出 `NOT_APPLICABLE`（含 reason），自含检查（摘要结构/正文格式/
  图表题注/三线表/页码/目录域/参考文献/附录/致谢/TF-20 渲染复核）照常执行；
