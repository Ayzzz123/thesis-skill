...
## 11. 旧版 .doc（Word 97-2003）学校材料处理（v1.3.11）

学校材料可能是 OLE2 旧格式（.doc，魔数 `D0CF11E0A1B11AE1`），**不能**直接作为
OOXML 母版，`select_docx_mode()` 对其返回 FORMAT_RECONSTRUCTION 属正确行为，
但**必须**先走以下预处理流程（不得静默忽略）：

1. **检测**：`detect_legacy_docs(school_dir)` 列出所有 OLE2 .doc 材料。
2. **转换**：`convert_legacy_doc(src, out_dir)` 用 Word COM 另存为 .docx 工作副本
   （原文件只读不动；产物放 `.aeromech/artifacts/`，不得放进 `materials/`）。
   环境缺 Word/pywin32 时抛 RuntimeError → 降级：请用户手工另存为 .docx 后登记。
3. **判定产物性质**（关键分岔）：
   - 含封面对象树 / 可继承样式 / 节结构（可作母版）→ 将转换产物登记为模板并走
     TEMPLATE_FIDELITY（模板来源注明"经旧版 .doc 转换"）；
   - 纯规范/文字文档（无对象树、无自定义样式）→ 解析全文为 school-format，
     走 FORMAT_RECONSTRUCTION；**format_source 仍按来源取 `school_template`**
     （学校正式文件）；格式参数 100% 取规范明文，规范未明说处标【学校格式待核实】。
4. **解析纪律**：Word 2003 文档常携带 smartTag（智能标记），其中可能包裹数字文本；
   python-docx `paragraph.text` 不遍历 smartTag 会**静默丢字**。逐段取文必须用
   lxml 全遍历 `.//w:t`；发现 smartTag 时在解析报告中记录该陷阱。
5. **视觉验证**：转换产物导出 PDF 渲染关键页（封面/格式描述页）人工核对，
   不得只凭 XML 判定。封面下划线填写槽等形态以渲染页为准。
