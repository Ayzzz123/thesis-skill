...
> 版本：v1.6.5（Academic Figure Quality & User-Configured Image Provider：在 v1.6.0 编排闭环之上，图形质量与可选外部生图——① 学术视觉系统（`figure_style.py` 单一来源：3 轮量化验证的调色板/字号阶梯/间距/图类型；`figkit.py` role= 语义角色色 + style_fingerprint 跨图家族一致；mermaid 回退 fail-closed 不伪造语义，自证式 PASS 移除）；② Visual QA（`figure_visual_qa.py` VIS-01~12：可测项机器判定、主观项→人工复核、分数永不覆盖 Critical；入 thesis_build QA 链 + Delivery Gate figure_visual 域）；③ User-Configured Image Provider（可选增强，零阻塞：`image_config.py` 凭据解析链 process→skill→`~/.aeromech/.env` 用户级、project .env 默认关闭、SecretStr 全掩码；`image_cli.py` config/status/test/remove）；④ OpenAI 兼容外部生图（`image_backends.py`：/images/generations b64、原子落盘、8 类错误分类——401=INVALID_CREDENTIAL 不伪装普通失败；可注入 transport，回归零真实网络）；⑤ No-Key Fallback（无 Key=正常状态自动回落既有 Figure Pipeline，UNAVAILABLE≠错误；旧 spec 零 env 接触与 v1.6.0 字节一致；五类确定性图恒 local 优先）；⑥ 受控 AI 生图（`ai_figure_gate.py`：Figure Plan 九字段前置，缺 Plan=FIGURE_PLAN_REQUIRED+零 HTTP；结构化溯源提示词+prompt_hash；provenance 白名单不含凭据；IMAGE_MAX_ATTEMPTS=3 成本闸+首次费用明示）；⑦ AI 生成视觉≠研究证据（research_integrity：ai_generated_visual 禁 verified/partial，证据种子仅 E/DS/CALC/M）；⑧ Secret Security（`secret_leak_qa.py` repo/diff/artifacts 三 scope，发布态 FAIL=0；record/log 边界 redact）；兼容 v1.0~v1.6.0 全部能力，只增不破；发布验证：tests/v1_6_5 124 断言 + v1_4/v1_4_1/v1_5/v1_6/test_a/test_b 全绿 + test-8.0 全 QA PASS + Delivery Gate=PASS + secret_leak_qa FAIL=0 + dev↔install IDENTICAL；已知限制：LIVE_SMOKE_TEST=NOT_RUN（USER_CREDENTIAL_NOT_PROVIDED，非阻塞））
>
...
## 19. Research Integrity Layer（v1.4.0）

完整规则见 `references/research-integrity.md`（注册表 Schema、ID 规则、RQG-01~15 定义、材料优先级、冲突处理、通用性纪律）。要点：

**定位**：v1.4 不是新增孤立模块，而是为 S3-S9 增加"研究逻辑 + 证据链 + 数据可追溯"层；与既有 QA（Format/Visual/Content Purity/Citation/Data Integrity）并列汇入 Delivery Gate。

**注册表**（`.aeromech/research/`，10 个 YAML + 生成的 `traceability.json`）：

```
rq.yaml        RQ-01   研究问题（question/objective/related_methods/chapters/analyses/conclusions）
methods.yaml   M-001   方法（name/basis 选择依据）
evidence.yaml  E-001   证据（source_type ∈ literature|standard|manual|official_document|
                            project_material|experiment|calculation|simulation|assumption；
                            verification_status ∈ verified|partial|pending|simulated）
datasets.yaml  DS-001  数据（含 Synthetic Data Ledger：reason/assumptions/generation_method/
                            parameters/limitations/label）
analyses.yaml  AN-001  分析（inputs 必须指向 DS/E/CALC；outputs 指向 TABLE/FIG）
computations.yaml CALC-001 计算（inputs/formula/operation/output/verification/used_in）
claims.yaml    CL-001  论断（claim_type ∈ fact|interpretation|calculation_result|
                            engineering_judgement|assumption|simulation_result）
conclusions.yaml CON-001 结论（claims/analyses/evidence 回溯链）
figures.yaml   FIG-001/TABLE-001 图表（related_rqs/analyses/claims 论证链接）
conflicts.yaml CONFLICT-001 冲突（source_a/source_b/conflict/resolution/reason/status）
```

**引擎与检查器**：

```bash
python scripts/research_integrity.py <root> init       # 初始化注册表
python scripts/research_integrity.py <root> validate   # 结构/悬空引用/伪装检测（RI-E-DISGUISE=Critical）
python scripts/research_integrity.py <root> trace      # 生成/更新 traceability.json（节点/边/orphans）
python scripts/research_integrity.py <root> coverage   # Evidence Coverage Ratio（simulated 单列，不并入 verified）
python scripts/research_quality_qa.py --project <root> [--pdf 毕业论文.pdf]   # RQG-01~15；0=PASS 1=FAIL 2=未初始化
```

**阶段职责**（写在哪）：S3 建 `rq.yaml`+`methods.yaml`；S4 文献→`evidence.yaml`；S5/S6 数据→`datasets.yaml`、分析→`analyses.yaml`、计算→`computations.yaml`（每次关键计算写 CALC 条目，替代旧 computation 审计的单一形态）；S7 每章核心论断注册 `claims.yaml` 并登记 `figures.yaml` 论证链接；写作完成注册 `conclusions.yaml`；S9 前跑 RQG 全量。

**四条不可协商规则**：

1. **模拟数据身份保持**：`type=simulated/assumption` 的数据集必须带 `label: 【假设/模拟·仅演示方法】`，且该标记出现在正文/表题/表注 ≥1 次；模拟数据不得被表述为"实测/试验/机队统计/实际运行"；不得在后续写作中丢失"模拟"身份（RQG-06/09/10 拦截）。
2. **证据越界判定**：证据全为 simulated/pending 时，论断/结论出现强断言口径（机队/实测/发生率为/统计表明等）→ Critical（证据越界），阻止交付；判定含否定语境豁免（"不代表真实机队统计特征"式免责句不触发）。
3. **结论可追溯**：每条核心结论必须有 claims/analyses 回溯链；无分析支撑 → High；结论明显超出证据能力 → Critical。
4. **计算可复现**：每个关键计算（RPN/统计量/单位换算/工程参数）注册 CALC 条目，inputs 指向 DS/E/CALC 且 formula+verification 齐备；正文重要数字应可追溯到引用文献编号或计算输出（RQG-13，≥50% 达标）。

**材料优先级**（证据冲突时）：学校正式要求 > 导师明确要求 > 用户项目材料 > 官方技术资料 > 可靠学术文献 > 公开行业资料 > 模板样例 > Skill 默认知识。模板样例只用于格式与表达参考，不得继承其数据/结果/结论/案例/引用事实。信息冲突必须登记 `conflicts.yaml` 并给 resolution+reason，无法判断时写【待核实】，**不得静默选择**。

**旧项目兼容**：无 `.aeromech/research/` 的项目不阻塞（`research_quality_qa.py` 退出码 2 + 报告记 not_initialized）；新项目从 S3 起必须建立。测试：`tests/v1_4/`（11 个测试文件 + run_all.py）。

### 19.1 v1.4.1 增补：状态模型 / NHR 人工复核 / 无模板 QA

1. **统一状态词表**（细则 `research-integrity.md` §19）：`PASS / WARN / FAIL / NEEDS_HUMAN_REVIEW / NOT_APPLICABLE / SKIPPED_WITH_REASON / ERROR`；FAIL 必须附 severity+reason+remediation；异常路径（ERROR）退出码 3，绝不产生 PASS。
...
**定位**：v1.6 不新增研究能力，而是把 v1.0~v1.5 的能力串成可恢复、可交付的完整闭环；编排层只做 routing/state/coordination/checkpoint/recovery，业务逻辑一律复用原模块。

**主循环**：`LOAD STATE → LOAD CONTEXT → ROUTE → CHECK PREREQS → EXECUTE → SAVE → CHECKPOINT → VALIDATE → NEXT`；失败 `DETECT→DIAGNOSE→RECOVERY→VALIDATE`；NEEDS_HUMAN_REVIEW 走既有双队列；Critical 一律 BLOCK（不得 retry 掩盖、评分不得放行）。

**入口**：`python scripts/thesis_orchestrator.py <root> status|step|drive|resume|run-loop|apply-human|gate|actions`。所有动作写 `artifacts/analysis/orchestrator-actions.jsonl`（action_id/stage/reason/input/output/status/timestamp）——禁止静默执行。

**四条不可协商规则**：

1. **状态机是唯一真源且经程序校验**：迁移只走 `thesis_state.transition`（非法边拒绝不落盘；revert 需触发源；override 需确认记录并挂 open_issue）；checkpoint（CK-XXX：stage/task/cursor/artifacts+sha256/registries 快照/qa_state/next_action）是 resume 依据；**禁止从 S1 重启**。
2. **Drift 不静默**：resume/step 先查 STATE_DRIFT / REGISTRY_DRIFT（注册表超前 state）/ ARTIFACT_MISSING(ERROR) / ARTIFACT_CHANGED(NHR)；发现差异交人工，绝不悄悄改写 state 或产物。
